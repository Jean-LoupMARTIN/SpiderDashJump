...
public class Spider : MonoBehaviour
{
    ...
    public void UpdateOrbits(bool gizmo = false)
    {
        if (updateOrbitsMethode == UpdateOrbitsMethode.LegDistance)
            for (int i = 0; i < orbits.Length; i++)
                UpdateOrbitLegDistance(orbits[i], orbits[i].parent, legs[i], orbitsLegsDists[i], gizmo);
    ...
    void UpdateOrbitLegDistance(Transform orbit, Transform orbitOrigin, Transform leg, float distLeg, bool gizmo = false)
    {
        Vector3 pos = orbitOrigin.position;
        Quaternion rot = orbitOrigin.rotation;
